...
filenamestr4 = 'conscan_accum_data.csv'; % BUG: Need to include path to z: drive
...
%
% Introduce clipping range
%RssiData = RssiData(24000:25500,:);
...
for Latency = -10:10 % Positive values imply RSSI data is delayed
    ...
    SiftedData   = YawData(logical(FullSieve),:);
    % Only keep data points where RSSI has not "bottomed-out"
    StrongSieve  = (SiftedData(:,3) > 1.05);
    PitchSieve   = (abs(SiftedData(:,2)) < 0.1);
    SiftedYaw = SiftedData((StrongSieve & PitchSieve),1);
    SiftedRSSI= SiftedData((StrongSieve & PitchSieve),3);
...
DoubledSieve = GoodLineSieve(11:(RowMax-10)) & GoodLineSieve((11+Latency):(RowMax+Latency-10));
FullSieve    = [ones(10,1); DoubledSieve; ones(10,1)];
SiftedData   = PitchData(logical(FullSieve),:);
StrongSieve  = (SiftedData(:,3) > 1.05);
YawSieve     = (abs(SiftedData(:,1)) < 0.1);
SiftedPitch  = PitchData((StrongSieve & YawSieve),2);
SiftedRSSI   = PitchData((StrongSieve & YawSieve),3);
% Compute parameters for best-fitting parabolic curve to good data
